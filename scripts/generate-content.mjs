...
function normalizeSecondSource(snapshot, seenRepositories) {
  ...
  if (plugins.length !== 8522) {
    fail(`expected 8522 second-source entries, found ${plugins.length}`);
  }
...
function normalizeReviewedAdditions(snapshot, seenRepositories) {
  ...
  if (records.length !== 10) {
    fail(`expected 10 reviewed catalog additions, found ${records.length}`);
  }
...
const expectedPluginCount = 8560 + upstreamAwesomeDeepseekHarnessSnapshot.records.length;
