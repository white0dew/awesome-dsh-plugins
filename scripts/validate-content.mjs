...
if (!Array.isArray(reviewedAdditionsInput.records) || reviewedAdditionsInput.records.length !== 10) {
  errors.push("reviewed catalog additions input must contain exactly 10 records");
}
...
const expectedPluginCount = 8560 + upstreamAwesomeDeepseekHarnessInput.records.length;
...
if (reviewedAdditionsByRepository.size !== 10) {
  errors.push(`expected 10 unique reviewed catalog additions, found ${reviewedAdditionsByRepository.size}`);
}
