...
if (!Array.isArray(reviewedAdditionsInput.records) || reviewedAdditionsInput.records.length !== 12) {
  errors.push("reviewed catalog additions input must contain exactly 12 records");
}
...
const expectedPluginCount = 8562 + upstreamAwesomeDeepseekHarnessInput.records.length;
...
if (reviewedAdditionsByRepository.size !== 12) {
  errors.push(`expected 12 unique reviewed catalog additions, found ${reviewedAdditionsByRepository.size}`);
}
