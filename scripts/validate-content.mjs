...
if (!Array.isArray(reviewedAdditionsInput.records) || reviewedAdditionsInput.records.length !== 11) {
  errors.push("reviewed catalog additions input must contain exactly 11 records");
}
...
const expectedPluginCount = 8561 + upstreamAwesomeDeepseekHarnessInput.records.length;
...
if (reviewedAdditionsByRepository.size !== 11) {
  errors.push(`expected 11 unique reviewed catalog additions, found ${reviewedAdditionsByRepository.size}`);
}
