...
if (!Array.isArray(reviewedAdditionsInput.records) || reviewedAdditionsInput.records.length !== 5) {
  errors.push("reviewed catalog additions input must contain exactly 5 records");
}
...
const expectedPluginCount = 367 + upstreamAwesomeDeepseekHarnessInput.records.length;
...
if (reviewedAdditionsByRepository.size !== 5) {
  errors.push(`expected 5 unique reviewed catalog additions, found ${reviewedAdditionsByRepository.size}`);
}
